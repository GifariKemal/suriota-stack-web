"""Legal v3 - match actual HTML-interleaved storage format."""
import sys, io, json, urllib.request, base64, time
sys.stdout = io.TextIOWrapper(sys.stdout.buffer, encoding='utf-8', errors='replace')
AUTH = base64.b64encode(b'admin:hCYK JqF1 khdB WDzI LQdQ WEBr').decode()
HDRS = {'Authorization':f'Basic {AUTH}','User-Agent':'Mozilla/5.0','Content-Type':'application/json'}

# Match by inner text fragments that are NOT broken by HTML
# These should not span across HTML tags
TRANS = {
    # Privacy page
    ' if you create an account on the SURGE platform, we collect your username, hashed password, and authentication tokens.':
        ' jika Anda membuat akun di platform SURGE, kami mengumpulkan nama pengguna, kata sandi ter-hash, dan token autentikasi Anda.',
    ' emails, WhatsApp messages, and call logs you exchange with our team.':
        ' email, pesan WhatsApp, dan log panggilan yang Anda pertukarkan dengan tim kami.',
    ' for IoT deployments using the SURGE platform, we collect device identifiers, sensor readings, geolocation (when consented), and event logs strictly for the purpose of operating the service.':
        ' untuk deployment IoT menggunakan platform SURGE, kami mengumpulkan identifier perangkat, pembacaan sensor, geolokasi (jika disetujui), dan log event semata-mata untuk tujuan mengoperasikan layanan.',
    ' - to fulfil engagement and service agreements.':
        ' - untuk memenuhi perjanjian engagement dan layanan.',
    ' - to operate our business, secure our services, and develop our products.':
        ' - untuk mengoperasikan bisnis kami, mengamankan layanan kami, dan mengembangkan produk kami.',
    ' - for marketing and optional cookies. You may withdraw consent at any time.':
        ' - untuk pemasaran dan cookie opsional. Anda dapat menarik persetujuan kapan saja.',
    ' - to comply with tax, accounting, and regulatory requirements.':
        ' - untuk mematuhi persyaratan pajak, akuntansi, dan regulasi.',
    ' processing data on our behalf (cloud hosting, email delivery, analytics, payment) under written data-processing agreements.':
        ' yang memproses data atas nama kami (cloud hosting, pengiriman email, analytics, pembayaran) berdasarkan perjanjian pemrosesan data tertulis.',
    ' integrators, certified installers, or auditors involved in delivering your proyek, only as necessary.':
        ' integrator, installer bersertifikat, atau auditor yang terlibat dalam menjalankan proyek Anda, hanya sesuai keperluan.',
    ' in the event of merger, acquisition, or asset transfer, with continuity of this Policy\\u2019s protections.':
        ' dalam hal merger, akuisisi, atau transfer aset, dengan kelanjutan perlindungan Kebijakan ini.',
    ' to understand traffic and improve content (e.g., Google Analytics). You may opt out via your browser settings.':
        ' untuk memahami trafik dan meningkatkan konten (mis. Google Analytics). Anda dapat opt-out melalui pengaturan browser.',
    ' only set with your explicit consent (where applicable).':
        ' hanya diatur dengan persetujuan eksplisit Anda (jika berlaku).',
    ' up to 24 months from last interaction.':
        ' hingga 24 bulan sejak interaksi terakhir.',
    ' for the duration of the engagement plus 10 years (Indonesian commercial-records requirement).':
        ' selama durasi engagement plus 10 tahun (persyaratan catatan komersial Indonesia).',
    ' as defined in the service-specific data-processing agreement.':
        ' sebagaimana didefinisikan dalam perjanjian pemrosesan data spesifik layanan.',
    ...
    ' obtain confirmation of and a copy of your data we hold.':
        ' memperoleh konfirmasi dan salinan data Anda yang kami simpan.',
    ' receive your data in a structured, machine-readable format.':
        ' menerima data Anda dalam format terstruktur dan terbaca mesin.',
    ' object to processing based on legitimate interests, termasuk profiling for marketing.':
        ' menolak pemrosesan berdasarkan kepentingan sah, termasuk profiling untuk pemasaran.',
    ' with Indonesia\\u2019s Personal Data Protection Agency or your EU supervisory authority.':
        ' kepada Lembaga Pelindungan Data Pribadi Indonesia atau otoritas pengawas UE Anda.',
    'To exercise these rights, contact ': 'Untuk menggunakan hak-hak ini, hubungi ',
    '. We will respond within 30 calendar days.': '. Kami akan merespons dalam 30 hari kalender.',
    'This policy is aligned with Indonesia\\u2019s Personal Data Protection Law (UU PDP No.27/2022) and references international standards such as the EU GDPR.':
        'Kebijakan ini sejalan dengan UU Pelindungan Data Pribadi Indonesia (UU PDP No.27/2022) dan merujuk pada standar internasional seperti EU GDPR.',
    ...
    # Terms page
    ' engineering, integration, consultation, and SaaS offerings provided by SURIOTA, termasuk the SURGE platform.':
        ' engineering, integrasi, konsultasi, dan penawaran SaaS yang disediakan oleh SURIOTA, termasuk platform SURGE.',
    ' any non-public information disclosed by either party that should reasonably be understood to be confidential.':
        ' informasi non-publik apapun yang diungkap oleh salah satu pihak yang sepatutnya dipahami sebagai rahasia.',
    'All firmware, software, designs, schematics, documentation, trademarks, and know-how created or owned by SURIOTA - termasuk the SURGE platform and all Products - remain the exclusive property of SURIOTA.':
        'Seluruh firmware, perangkat lunak, desain, skematik, dokumentasi, merek dagang, dan know-how yang dibuat atau dimiliki SURIOTA - termasuk platform SURGE dan semua Produk - tetap menjadi milik eksklusif SURIOTA.',
    ' as quoted in writing. Quotes are valid for 30 days unless extended.':
        ' sesuai penawaran tertulis. Penawaran berlaku 30 hari kecuali diperpanjang.',
    ' VAT (PPN), withholding tax (PPh), and other applicable taxes are added to invoiced amounts unless quoted as tax-inclusive.':
        ' PPN, PPh (withholding), dan pajak lain yang berlaku ditambahkan ke jumlah faktur kecuali ditawarkan inclusive pajak.',
    ' we may suspend services and charge interest at 1.5% per month on overdue balances, to the extent permitted by law.':
        ' kami dapat menangguhkan layanan dan mengenakan bunga 1,5% per bulan atas saldo jatuh tempo, sejauh diperbolehkan hukum.',
    'Each party will protect the other\\u2019s Confidential Information with the same degree of care it uses to protect its own (no less than reasonable care), use it solely to perform the Engagement, and not disclose it except to those with a need to know who are bound by similar confidentiality obligations.':
        'Setiap pihak akan melindungi Informasi Rahasia pihak lain dengan kehati-hatian yang sama seperti melindungi miliknya sendiri (tidak kurang dari kehati-hatian wajar), menggunakannya semata-mata untuk melaksanakan Engagement, dan tidak mengungkapkannya kecuali kepada mereka yang perlu tahu dan terikat kewajiban kerahasiaan serupa.',
    ...
    ' these Terms (together with any Engagement and our Kebijakan Privasi) constitute the entire agreement.':
        ' Syarat ini (bersama dengan Engagement apapun dan Kebijakan Privasi kami) merupakan keseluruhan perjanjian.',
    ' SURIOTA may update these Terms; material changes will be notified at least 14 days in advance.':
        ' SURIOTA dapat memperbarui Syarat ini; perubahan material akan diberitahukan paling lambat 14 hari sebelumnya.',
    ' if any provision is held unenforceable, the remainder remains in effect.':
        ' jika ada ketentuan yang tidak dapat ditegakkan, sisanya tetap berlaku.',
    ' you may not assign these Terms without our written consent; SURIOTA may assign to an affiliate or successor.':
        ' Anda tidak boleh mengalihkan Syarat ini tanpa persetujuan tertulis kami; SURIOTA boleh mengalihkan kepada afiliasi atau penerus.',
    ' neither party is liable for delays caused by events beyond reasonable control (natural disasters, war, pandemic, government action).':
        ' tidak ada pihak yang bertanggung jawab atas keterlambatan akibat peristiwa di luar kendali wajar (bencana alam, perang, pandemi, tindakan pemerintah).',
    ' written notices to admin@suriota.com for SURIOTA, or to the address you provided.':
        ' pemberitahuan tertulis ke admin@suriota.com untuk SURIOTA, atau ke alamat yang Anda berikan.',
...
$log = $upload['basedir']."/purge-legal-v3.txt";
...
"""
urllib.request.urlopen(urllib.request.Request('https://suriota.com/wp-json/code-snippets/v1/snippets/5', data=json.dumps({'code':purge, 'active':True, 'name':'SX: legal v3'}).encode(), method='POST', headers=HDRS), timeout=60).read()
time.sleep(3)
